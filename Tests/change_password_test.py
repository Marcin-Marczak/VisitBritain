...
class TestChangePassword(ChangePasswordSetup):
    ...
    def test_change_password_too_short_new_password(self):
        ...
        base_page = BasePage(self.driver)
        validation_error_texts = base_page.base_page_get_validation_error_texts()
        assert validation_error_texts[0].get_attribute("textContent") == data("validation_error_texts.json")["too_short_new_password_error_text"]
        assert validation_error_texts[1].get_attribute("textContent") == data("validation_error_texts.json")["confirm_new_password_doesnt_match_new_password_error_text"]
